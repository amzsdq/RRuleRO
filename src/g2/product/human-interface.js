...
function progressSummary(input = {}) {
  ...
  const inferred = active.length || remaining.length ? 'CONTINUE' : 'COMPLETE';
  const explicit = text(input.status);
  if (explicit === 'COMPLETE' && (active.length || remaining.length)) throw new Error('COMPLETE progress cannot contain active or remaining work');
  return Object.freeze({ status: explicit || inferred, completed: Object.freeze(completed), active: Object.freeze(active), remaining: Object.freeze(remaining), next: text(input.next), summary: [completed.length ? `Completed: ${completed.join('; ')}` : '', active.length ? `Active: ${active.join('; ')}` : '', remaining.length ? `Remaining: ${remaining.join('; ')}` : '', text(input.next) ? `Next: ${text(input.next)}` : ''].filter(Boolean).join('\n') });
}
...
function blockerReport(input = {}) {
  ...
  if (!action) throw new Error('blocker action required'); if (!reason) throw new Error('blocker reason required'); if (!resume) throw new Error('resume_from required');
