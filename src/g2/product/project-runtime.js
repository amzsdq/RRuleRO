...
function compile(plan, input) {
  return compileWorkItems({ plan, objective_ref: input.objective_ref, subject_sha: input.subject_sha, verification_contract: input.verification_contract, domain_prefix: input.domain_prefix });
}
...
function approveProject(project, input = {}) {
  ...
  return Object.freeze({ ...project, state: 'RUNNING', plan, work_items: compile(plan, input) });
}

function reviseProjectFromEvidence(project, input = {}) {
  if (!project || project.type !== 'g2-product-project') throw new Error('valid product project required');
  if (project.state === 'AWAIT_APPROVAL') throw new Error('project cannot be revised from evidence while awaiting approval');
  ...
  const revised = planApi.revisePlan(project.plan, { reason: input.reason || 'new evidence changed execution plan', evidence_refs: incoming.map((x) => x.id), steps: input.steps });
  const plan = planApi.continuePlan(revised);
...
function reviseProjectFromUser(project, input = {}) {
  if (!project || project.type !== 'g2-product-project') throw new Error('valid product project required');
  if (!['AWAIT_APPROVAL', 'RUNNING'].includes(project.state)) throw new Error('project cannot be revised');
  const revised = planApi.revisePlan(project.plan, { reason: input.reason || 'user revised plan', steps: input.steps });
  if (project.state === 'AWAIT_APPROVAL') {
    return Object.freeze({ ...project, plan: revised, work_items: Object.freeze([]) });
  }
  const plan = planApi.continuePlan(revised);
