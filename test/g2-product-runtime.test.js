...
test('planned work keeps preview informational and non-blocking', () => {
  const result = P.intake.compileIntent({
    intent: 'implement a multi-stage runtime improvement',
    multi_stage: true,
    material_research_value: true
  });
...
test('approval is reserved for genuine decision or risk gates', () => {
  const result = P.intake.compileIntent({
    intent: 'perform irreversible external mutation',
    irreversible: true
  });
...
test('blocked path continues through another safe runnable path', () => {
  const result = P.blocker.resolveBlocker({
    blocked: true,
    alternate_paths: [
      { id: 'waiting-path', runnable: false },
      { id: 'independent-useful-work', runnable: true, safe: true }
    ],
...
test('blocked status is terminal for turn only after alternates are exhausted', () => {
  const result = P.blocker.resolveBlocker({
    blocked: true,
    alternate_paths: [{ id: 'unsafe', runnable: true, safe: false }],
    user_action_possible: true
  });
...
test('turn reporting refuses fabricated useful time beyond elapsed runtime', () => {
  assert.throws(() => P.reporting.renderTurnReport({
...
test('turn reporting keeps concise machine-readable minimum fields', () => {
  const report = P.reporting.renderTurnReport({
    start: '2026-01-01T00:00:00Z',
    end: '2026-01-01T00:10:30Z',
    worked_ms: 600000,
    status: 'CONTINUE',
    completed: 'product intake',
    next: 'blocker handling'
  });
...
test('recursive improvement accepts DO_NOTHING when no change has positive external value', () => {
  const result = P.recursive.selectImprovement([
    { action: 'ADD', expected_benefit: 1, control_cost: 2, risk_cost: 0, measurable_outcome: 'extra logs' }
  ]);
