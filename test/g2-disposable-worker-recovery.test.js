...
function workItem() {
  return {
    ...
    useful_work_score: 10,
    coordination_cost_score: 1,
    estimated_minutes: 5
  };
}
...
test('a replacement Worker cold-resumes from durable checkpoint after the original Worker disappears', async () => {
  ...
  const queue = createDurableForemanQueue({
    durable_state: durable,
    queue_id: 'queue-recovery',
    subject_sha: 'subject-recovery-1'
  });
  ...
  const checkpoint = resumableCheckpoint(original, {
    next_action: 'continue-recovery-fixture',
    durable_refs: ['work:work-recovery-1']
  }, 2000);

  const persisted = await durable.commit({
    logical_id: 'objective-recovery-1',
    generation: 1,
    subject_sha: 'subject-recovery-1',
    status: 'WORKER_CHECKPOINT',
    checkpoint,
    durable_refs: ['work:work-recovery-1']
  });

  // The original in-memory Worker is deliberately discarded here.
  original = null;
  ...
  const resumed = evolve(replacement, {
    next_state: 'WAITING_VERIFICATION',
    checkpoint: true,
    useful_units_delta: 2,
    checkpoint_ref: 'durable:checkpoint-2'
  });
