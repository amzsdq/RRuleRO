...
## Variant A — CONTROL / long single Foreman
Keep the current long self-contained Foreman prompt and current single-relay behavior.
This is the baseline, not the assumed winner.

Hypothesis: highest local instruction redundancy, but strongest cold-start resilience.
Expected weakness: prompt/control overhead and repeated policy parsing.
